/**
 * @file app/api/tools/recipes/[id]/duplicate/route.ts
 * @purpose API: Rezept duplizieren.
 * @route POST /api/tools/recipes/{id}/duplicate
 */

import {
  buildRecipeMembershipContext,
  resolveRecipeUserId,
} from "@/lib/auth/recipe-request-auth";
import {
  jsonFromServiceResult,
  jsonSuccess,
  parseJsonBody,
} from "@/lib/tools/recipe-api-utils";
import { recipeFailure } from "@/lib/tools/recipe-errors";
import { duplicateRecipe } from "@/lib/tools/recipe-service";

type RouteContext = {
  params: Promise<{ id: string }>;
};

/**
 * POST — Kopie eines Rezepts für den anfragenden Nutzer erstellen.
 * Body: { userId }
 */
export async function POST(
  request: Request,
  context: RouteContext,
): Promise<Response> {
  const { id } = await context.params;
  const body = await parseJsonBody(request);

  if (!body) {
    return jsonFromServiceResult(
      recipeFailure({
        code: "VALIDATION_ERROR",
        message: "Ungültiger JSON-Body.",
      }),
    );
  }

  const userId = await resolveRecipeUserId(request, body);

  if (!userId) {
    return jsonFromServiceResult(
      recipeFailure({
        code: "VALIDATION_ERROR",
        message: "Feld userId ist erforderlich.",
      }),
    );
  }

  const membership = await buildRecipeMembershipContext(request, userId, body);
  const result = await duplicateRecipe(id, userId, membership);

  if (result.success) {
    return jsonSuccess(result.data, 201);
  }

  return jsonFromServiceResult(result);
}
