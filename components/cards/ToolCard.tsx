 ...
 * Tools mit `comingSoon` zeigen keinen Link.
 */
export default function ToolCard({ tool }: { tool: Tool }) {
  ...
  return (
    <article
      className={`group flex flex-col rounded-xl border border-aw-border bg-aw-surface p-5 transition-colors ${
        isComingSoon ? "opacity-90" : "hover:border-aw-gold/50"
      }`}
    >
      <div className="flex items-start justify-between gap-2">
        <span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-aw-surface-2 text-aw-gold ring-1 ring-aw-border">
          <Icon name={tool.icon} />
        </span>
        <div className="flex flex-wrap justify-end gap-1.5">
          {isComingSoon && (
            <span className="rounded-full bg-aw-surface-2 px-2.5 py-1 text-xs font-medium text-aw-muted ring-1 ring-aw-border">
              Coming Soon
            </span>
          )}
          <span
      ...
      {isComingSoon ? (
        <p className="mt-4 text-sm font-medium text-aw-muted">In Kürze verfügbar</p>
      ) : (
        <Link
          href={href}
