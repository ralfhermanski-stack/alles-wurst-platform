...
export const LEGAL_CRON_ROUTE = "/api/cron/legal-documents";
export const LEGAL_CRON_METHOD = "POST";
export const LEGAL_CRON_AUTH_HEADER =
  "Authorization: Bearer <LEGAL_CRON_SECRET>";

export function authorizeLegalCron(request: Request): {
  authorized: boolean;
  status: 401 | 429;
} {
  ...
  const secret = process.env.LEGAL_CRON_SECRET?.trim();
